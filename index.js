  ...
  btn.addEventListener('click' , () => {
   const data = {}
    ...
    if (btn.parentElement.parentElement.querySelector('img')){
      data.image = btn.parentElement.parentElement.querySelector('img').getAttribute('src')
      document.querySelector('.card-contents img').setAttribute('src' , data.image)
    } else {
      data.image = window.getComputedStyle(btn.parentElement).getPropertyValue('background-image').split('/')
      document.querySelector('.card-contents img').setAttribute('src' , './workImg/' + data.image[data.image.length-1].replace('")',''))
    }
    ...
    if (btn.parentElement.querySelector('ul')){
      ...
      for (let i = 1; i<= data.list.split('\n').length; i++){
        document.querySelector('.popup-card ul').innerHTML += "<li> "+ data.list.split('\n')[i-1] +" </li>" 
      }
    }
